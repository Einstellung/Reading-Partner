...
// reverse-DNS custom scheme captured by tauri-plugin-deep-link. Google retired the
// loopback redirect for native iOS/Android clients, and it forbids loading the
// consent screen in an embedded webview, so the scheme + system-browser route is
// the only path left on both mobile platforms.
//
// The two mobile flows are the same protocol and differ only in which client the
// credentials come from: a client is bound to one platform identity (iOS bundle
// id vs. Android package name + signing certificate SHA-1), so each needs its own
// id and therefore its own reversed scheme. They stay separate kinds so the flow
// a build is running is visible in a log or a test, not inferred.
...
export interface AuthFlow {
  kind: AuthFlowKind;
  clientId: string;
  // Present only for the desktop loopback flow; the mobile clients have no secret.
  clientSecret?: string;
  redirectUri: string;
}
...
// Turn a Google mobile client id into its reverse-DNS URL scheme:
//   "12345-abcdef.apps.googleusercontent.com" -> "com.googleusercontent.apps.12345-abcdef"
// This scheme is what the app registers statically (via tauri.conf.json
// plugins.deep-link.mobile — Info.plist on iOS, an intent-filter on Android) and
// what Google redirects to after authorization.
...
// Full redirect URI for a mobile client: "<reversed-client-id>:/oauth2redirect".
// Empty string when that client is not configured (so isGoogleConfigured stays
// false and the sign-in button is disabled instead of starting a broken flow).
...
//
// A mobile platform with no client id returns null rather than falling through to
// the desktop branch: Google refuses a loopback redirect for a mobile client type,
// and the desktop client's secret must not ship in a mobile bundle, so falling
// through would offer a sign-in button that cannot work.
...
// only for the desktop flow; the mobile public clients must not send one.
...
// Custom-scheme URLs arrive as "com.googleusercontent.apps.x:/oauth2redirect?...".
