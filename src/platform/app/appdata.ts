...
// The base every path below is resolved against. Not exported: a caller that
// needs it is a caller that is still talking to the plugin.
...
export const appData: AppDataFs = {
  ...
  readText(path) {
    return readTextFile(path, APPDATA);
  },
  ...
  writeBytes(path, bytes) {
    return writeFile(path, bytes, APPDATA);
  },
  ...
  appendText(path, text) {
    return writeTextFile(path, text, { ...APPDATA, append: true });
  },
  ...
  mkdirp(path) {
    return mkdir(path, { ...APPDATA, recursive: true });
  },
  ...
  remove(path) {
    return remove(path, APPDATA);
  },

  removeDir(path) {
    return remove(path, { ...APPDATA, recursive: true });
  },
