...
export interface ObservationToolOptions {
  ...
  // lands on disk — the message's id, or the legacy "<threadId>:<ts>" pair for
  // one stored before ids existed (anchors.ts) — and the day that line
  // happened. The model cites the number; the id and the date are read off this
  // table.
  //
