...
// rehearsal need it (docs/43) and getting it wrong is silent: the source has to
// be recording before the deck reports its first page, or every word said to
// page one is lost.
//
// A source that cannot be built is null, not a throw. A run that records pages
// and no words is a run (reading/rehearsal) — no STT key on the desktop and no
// dictation on the host are both ordinary states of this machine, and neither is
// a reason to refuse to put the deck on screen.
...
// The deck's name and whatever proper names the caller can give in advance go in
