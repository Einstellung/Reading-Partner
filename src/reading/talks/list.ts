...
// (slides/talks.json, keyed by the same talk id).

import type { TalkEntry as DeckEntry } from "../slides";
...
export function talkRows(talks: readonly Talk[], decks: readonly DeckEntry[]): TalkRow[] {
  const deckOf = new Map<string, DeckEntry>();
  for (const d of decks) if (d.talkId) deckOf.set(d.talkId, d);
  return talks
    .map((t): TalkRow => {
      const deck = deckOf.get(t.id) ?? null;
      return {
        id: t.id,
        name: t.name,
        createdAt: t.createdAt,
        updatedAt: t.updatedAt,
        stage: deck ? "deck" : "preparing",
