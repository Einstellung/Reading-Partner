...
// says when, in what order, in which language, what the plugin streamed while
// the finger was down and what each step of the press cost — which is what it
// takes to line the file up afterwards against a console log, a syslog, or
// someone's memory of what they said. The index and the session
...
export interface BenchEntry {
  ...
  heard: Heard | null;
  locale: string;
  /// Where the press went, as the plugin measured it: whether the microphone was
  /// inherited from the hold before it, milliseconds to each step of the start,
  /// milliseconds to each step of the teardown, and what the pre-roll held. Null
  /// when the hold produced none — a typed line, or a hold whose numbers had not
  /// crossed back by the time the row was made.
  ///
  /// The reason it is in the file at all: the `RP-DICT` lines that carry the
  /// same numbers are a debug build's only, and on the round this was written
  /// for they were the only road. The Mac's idevicesyslog stayed up with a dead
  /// stream behind it and twelve holds' worth of them were gone. A file on the
  /// phone is read afterwards, which is when it turns out whether the road was
  /// open.
