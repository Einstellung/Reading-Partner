...
export const PALACE = [
  ...
  },
  {
    kind: "info-dinner",
    domain: "info",
    match: fixed("info-dinner.json"),
    samples: ["info-dinner.json"],
    id: "fixed",
    refs: [],
    sync: "data",
    merge: "opaque",
  ...
  {
    kind: "info-dinner-photos",
    domain: "info",
    match: fixed("info-dinner-photos.json"),
    samples: ["info-dinner-photos.json"],
    id: "fixed",
    refs: [],
    sync: "data",
    merge: "opaque",
    deleteWith: "never",
    gc: "never",
    note: "the photographs the dinner line found for dishes and ingredients (docs/73 图片), keyed by what was searched for and outliving every week; its own file because the run that writes it is on the PC minutes after the phone wrote the week",
  },
  {
    kind: "info-picture",
    domain: "info",
