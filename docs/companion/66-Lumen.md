...
> 2026-09-10 定案。上游：[45](./45-陪伴的形态.md) 是形态的证据与法律边界，[61](../soul/61-palace与desk.md) 是 palace 与 desk，[48](../soul/48-记忆：观察与statement.md) 是观察与 statement，[55](../soul/55-legion.md) 是 run 与调度，[north-star/companion.md](../north-star/companion.md) 是形象那一层的愿景。
>
> 取代关系：[45](./45-陪伴的形态.md) 曾定的「一个球就是全部陪伴」已作废并删除，45 现在只留各节证据、「法律边界」和「将来升到形象时已知的」；2026-08-26 那张「电电」概念图定的八种情绪、五套日常动作、token 喂养的能量条全部作废。2026-09-15 由 [68](./68-Lumen与盒子的交互.md) 修订「位置与尺寸」：全局常驻、logo 开关、看书时静止、红盒站在旁边；「reading 侧的聊天入口要不要也露 Lumen 的脸」由 68 定掉，露。第三轮调研的结论吸收进本文（[round3-01](../assets/companion-research/round3-01-motion-controllers.md)、[round3-02](../assets/companion-research/round3-02-rig-and-motion-data.md)、[round3-03](../assets/companion-research/round3-03-ipad-runtime.md)、[round3-04](../assets/companion-research/round3-04-behavior-layer.md)）。

---

## 是谁

Lumen 是 soul（[61](../soul/61-palace与desk.md)）在空桌前的样子，不是另一只吉祥物。同一份记忆、同一条对话尾、同一套工具；桌上没有材料时的那次装配就是它，门（`src/soul/door.ts`）就是和它说话。代码和界面里叫 Lumen，中文对话里可以叫卢明。
...
它表达四件事：soul 的注意力在哪，它这会儿干得多重，对读者在材料上下的功夫的一声回应，以及有东西给读者——盒子站在它旁边，它只瞟一眼（[68](./68-Lumen与盒子的交互.md)）。
...
配色上 app 是纸（`#faf9f6`）加墨（`#252922`）、绿只画线（[52](../ui/52-配色.md)），Lumen 是画面里唯一发光的蓝白物体。app 今天没有暗色模式。
...
| 想 | 一次模型调用在飞 |
| 说 | TTS 包络 |
| 干活 | 一次工具调用或一个 legion run 在飞（[55](../soul/55-legion.md)） |
| 睡 | 夜里的 dream 与蒸馏在跑（[48](../soul/48-记忆：观察与statement.md)、[58](../soul/58-蒸馏器与dream.md)） |
| 找到了 | 刚蒸出一条观察 |
| 卡住了 | 模型在意图里自己说的 |
| 盒子到了 | 盒里新添了没开的项（[68](./68-Lumen与盒子的交互.md)）。眼睛往盒子瞟一下再看回来，一次，不是一段动画 |
...
喂"说"这一段的是那条包络不是麦克风：通话期间麦克风一直开着，它报的是屋里的声音，照它开嘴就是冲着旁边最响的东西开。包络按句从 Swift 过来（33），到 WebView 按本地时钟回放（`src/ui/components/lumen/envelope.ts`），一句的开头就是弹跳的触发；别的段仍然读麦克风电平。`attention` 由这一轮的工具调用驱动：有工具在飞就是 work，全部返回后按 600 ms 最短停留加 250 ms 去抖回 reader（`src/ui/components/lumen/attention.ts`），legion run 还没接。SoulIntent 里模型自报的那个 `attention` 字段仍是将来的事。
...
全局常驻：每个外壳每个地方的右下角都有，72 px，就是简报页球原来占的那个角。门那一层放大的不变，桌上什么都没有。全窗聊天里输入条占着下边缘时，角抬到输入条上面——按输入条量出来的高度抬（含安全区），不是估一个数；卡片列还是从角上往上开。只有按住说话的那一下让开：落点面板铺满输入条宽度，右边「Edit」压在角下面（[68](./68-Lumen与盒子的交互.md)）。

角上那个身体通话期间不再变大也不再居中。原来它会长到 160 px 挪到屏幕中间，等于每次通话开场都把一个身体甩到正在讲的东西上面。四段在 72 px 上都读得出来（56 px 上眉毛糊成一团、平嘴和笑嘴分不开），所以就待在角上。

点左上角的 Reading Partner logo 切换它出现或消失，按设备记住。阅读器里没有 logo，开关是顶栏 More 菜单里的一行 `Lumen`。关掉时红盒一起关，项照攒（[68](./68-Lumen与盒子的交互.md)）。

红盒站在它左边，同一个光池里，只在盒里有没开的项时出现，空了就消失；外观和精确尺寸见 [68](./68-Lumen与盒子的交互.md) 的「Lumen 作为入口」。

看书时它静止：不呼吸不摇，也会出现在打开的书的正文旁边——45 里「动画放在叙事文字旁边把注意力拉走」的证据（45 的「为什么不是角色」一节，Wang et al. 2024-12-06）针对的是持续动画，一次性的状态变化不在其内，读者也可以把它整个关掉。唯一会变的是盒子到了那一下：眼睛往它瞟一下再看回来（用「查」那段的眼睛动作），之后继续静止。
