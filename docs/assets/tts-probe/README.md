...
2026-08-27 在 Linux 桌面机上量的 TTS 选型数据，三轮。结论吸收进 [33](../../33-语音简报.md) 的「首包延迟实测」（延迟）和「音质与读音实测」（音质、读音、小米）。实测发现结论不对时回来查这里。
...
`first_pcm_e2e_ms` 是首帧 PCM 到达，不是听见第一个字——句首静音要另外加（三家 85 / 375 / 117 ms，在 `quality-report-2026-08-27.md` 第三节）。33 里定的判据是：p90 都超过 500 ms 才要重想按句接力这个形态；小米那一份 p90 是 1042 ms，没过，为什么这条 FAIL 不否决它写在 33 的「音质与读音实测」。
