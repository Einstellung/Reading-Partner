...
pdf.js 用 iframe 加载，`src` 是相对路径 `pdf/web/viewer.html`。宿主页面必须和 `pdf/` 目录同级，pdf.worker、cmaps、字体、wasm 全走这个相对根。

View 用的 mobile pdf.js 裸调 `Math.sumPrecise` 且不 polyfill（reader 的 legacy web 构建才自带）。缺该方法的引擎上，文本层几何会抛 TypeError，划词/标注浮窗的 `rect` 拿不到。宿主页在 `view.js` 之前补一段 polyfill：
...
判断就绪用 `onInitialized` 回调。`view.initializedPromise`（View 顶层）是 undefined，只有内部的 `view._view.initializedPromise` 才 resolve，别依赖顶层的。

程序化 `selectAnnotations([id])` 和 `navigate({annotationID})` 不会打开标注浮窗——源码里只有真实点击才开，程序化选中反而发一次无参（关闭）。壳里"点某条标注弹 AI 解释"要走真实点击，或监听 `onSelectAnnotations` 自己定位。

构建有网络依赖：webpack 的 ZoteroLocalePlugin 会从 `raw.githubusercontent.com` 下载语言文件。离线或被墙会构建失败，需预缓存 `locales/` 或加代理。
