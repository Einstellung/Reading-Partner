...
红盒站在它左边，同一个光池里，72 px 的角上约 24×25 px（圆身子的六成高，底边和身体齐平），只在盒里有没开的项时出现，空了就消失（[68](./68-Lumen与盒子的交互.md)）。

看书时它静止：不呼吸不摇。唯一会变的是盒子到了那一下：眼睛往它瞟一下再看回来，之后继续静止。

2026-09-15 修订：原来那句「它永远不出现在打开的书的正文旁边」作废。45 里「动画放在叙事文字旁边把注意力拉走」的证据（45 的「为什么不是角色」一节，Wang et al. 2024-12-06）针对的是持续动画，一次性的状态变化不在其内；而且读者可以把它整个关掉。
