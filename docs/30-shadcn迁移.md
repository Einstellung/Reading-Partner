...
对话框是唯一一种层级不固定的浮层：它落在开它的那个面之上，而它 Portal 到 `body`，DOM 位置说明不了是谁开的。所以由面声明——手写的浮层（`CallBubble` 那批）把自己整棵子树包进 `<OverlaySurface layer="floating">`，`DialogContent` / `AlertDialogContent` 和它们的 backdrop 各自 `useDialogLayer()` 取值，backdrop 也要抬（暗片留在下面就是同一个 bug 小一号）。context 跟 React 树走，Portal 挡不住。不这么做的后果见坑 208：确认框被开它的弹层盖掉一半，Cancel 按不到。
