...
**层级**。`ui/overlay.tsx` 的 `OVERLAY_Z` 是全 app 唯一一条 z 阶梯，每层的数字只在那里写一次：toast 30、dialog 50、page 70、pageDialog 80、floating 1000、floatingTop 1001、floatingDialog 1050、anchored 1100、blocking 2000。要守的不变量是「锚定浮层画在它触发器所在的那层之上」，而触发器可以坐在阶梯的任何一层，所以 anchored 排在整条阶梯之上。`blocking` 只有迁移蒙层用（docs/48）：它的意思是身后一切都不许碰，所以连锚定层也压住。调用点不写数字，`className` 里取 `OVERLAY_Z.<层名>`；全屏页那层归 `DialogFullScreenContent` 自己。这条阶梯是坑 103 立起来的。
