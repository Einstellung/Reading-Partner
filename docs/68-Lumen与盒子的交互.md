...
盒子是角上独立的一层，不在 Lumen 手里：一个竖放的红色公文盒（英国大臣的 dispatch box 那种，红色哑光皮面、盒盖在上、正中一个铜扣，没有花押也没有文字；柔光 2.5D 和 Lumen 一致，但它自己不发光），站在 Lumen 左边、同一个光池里，约 40 px 高，比身体矮一头。只在有没开的项时出现，空了就消失，角上只剩 Lumen。手机宽度上 72 加 40 加间距不到 130 px。Lumen 的身体位图和 SVG 眼嘴不为盒子改动。
...
数字徽标在盒子右上角，用现有的墨绿徽标样式，数的是没开的项数。
