...
盒子的路线：起点是身体正中，整个矩形都盖在圆身子轮廓里，什么都不露；往左下走，走在身体后面，到不再和身体重叠的那一点换到身前（在不重叠处换层，才不会看见一下跳层），再往右下落进原位，最后这段从 0.92 放大到 1，像是朝人递过来。取出 550 ms，放回 450 ms，进度是匀速时钟，缓动在路径里：中途箱子空了或又来一项，只改目标，盒子从当前位置掉头，不跳。这两下里眼睛盯着盒子、身体朝盒子那边倾几度，落定或收回后都回正。已经出来了再多一项，只改徽标数字。开 app 时箱里本来就有项的，盒子直接在原位，不演取出。prefers-reduced-motion 下没有路径也没有倾斜，盒子直接出现或消失。盒子只有落定后能点，路上不接指针也不进 tab 序。
