...
解法：宿主自己居中。`viewport.scrollTo` 支持 `alignX`（百分比，`finalX = x - clientWidth * alignX/100`），所以翻页时传 `alignX = 50 * (1 - 页宽px / 视口宽px)`（钳在 0..50，页比视口宽时为 0），页就落在屏幕正中，两侧对称露出邻页边缘——这反而是"还有下一页"的提示。页宽从 `scrollScope.getLayout().virtualItems` 里找到该页的 item 宽度乘当前 `currentZoomLevel` 得到。纯函数 `pageCenterAlign` 在 `src/reading/engine/gesture/paged-gesture.ts`，有单测。
