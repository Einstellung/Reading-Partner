...
- [198-detaching-a-node-from-a-running-engine-aborts](./198-detaching-a-node-from-a-running-engine-aborts.md) — engine 还在跑就 `detach` 节点，`AVAudioEngineGraph::RemoveNode` 抛的是 ObjC 异常而不是往 `NSError**` 里填错误，Swift 接不住，整个进程 abort；触发点是播放中途的路由变化、中断和进后台，现象是「腿卡住了、日志里什么都没有」。拆栈顺序改成摘 tap → 停 player → 停 engine → 最后 detach
