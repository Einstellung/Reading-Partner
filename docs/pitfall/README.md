...
| 裁 TTS 的头尾静音、按句拼接 | 原生音频与语音 |
| 把整段跑的纯函数搬进流式、切句 | 原生音频与语音 |
...
编号只加不回收：删掉的坑、或 2026-08-21 那次给撞号坑腾地方用掉的号，都不再复用；新坑接着当前最大编号往后加（下一个是 383）。
...
- [372-bing-images-serves-curl-a-decoy-page](./372-bing-images-serves-curl-a-decoy-page.md) — Bing 图片搜索对 curl 回 200、标题正确、`a.iusc` 结构完整，内容却是缓存里的无关结果（"mapo tofu" 出 cat memes），换个空格编码结果又不一样；解析器只能验形状，搜索本身必须在隐藏 webview 里跑，解析放页内脚本
...
- [210-a-nightly-pass-runs-once-per-entry-point](./210-a-nightly-pass-runs-once-per-entry-point.md) — dream 的日闸只看跑完才写的状态文件，一次启动里启动/回前台/定时器三个入口打进来跑了四遍，十条结论落盘两组（一中一英）。闸放在被调用的那一侧：进程内在飞标记 + 进程内当日标记
- [321-the-report-throttle-hides-every-phase-but-the-first](./321-the-report-throttle-hides-every-phase-but-the-first.md) — run 的 `progress` 每 30 秒才落一次盘，第一行之后、终态之前的相位变化在 run 文件上看不见；测试要把时钟推过窗口，产品别拿它当逐步进度条
...
- [191-tts-lead-out-is-room-tone-not-silence](./191-tts-lead-out-is-room-tone-not-silence.md) — 小米每句前后那 117/451 ms 不是数字静音，是 -45 到 -65 dBFS 的底噪，按 -60 dBFS 裁句尾平均只裁掉 39 ms（要裁到 451 得把阈值压到 -45）；护栏也不是保险——句尾护栏归零，「是。」被 ASR 回读成 `Shi.`，句首护栏归零一句没变。定下来的三个数：阈值 -45 dBFS、句首护栏 40 ms、句尾护栏 60 ms，整句没有一个超阈值窗时两端都不裁
- [192-mimo-200-stream-with-no-audio](./192-mimo-200-stream-with-no-audio.md) — 小米会返回 200 的 SSE 流、帧全合法、`[DONE]` 也来，但一帧音频都没有，约 30 次里 2 次，同一段文本重发就好；别当成内容审核拒绝（那条接口目前根本不审文本，9 句违法交易和政治敏感文本一次没拒），单列一种可重试的错误
...
- [344-the-fake-appdatas-readdir-ignores-the-directory](./344-the-fake-appdatas-readdir-ignores-the-directory.md) — `tests/support/appdata-fake.ts` 的 `readDir` mock 忽略传进来的路径，答的是整块盘的整条路径、`isFile` 恒为 true；扫子目录的 store（box、legion/runs、legion/bell、legion/ledger）在它上面列表恒为空，测试会因为错误的理由变绿。那四家一律拿 Map 当盘，假 AppData 上只钉路径、内容和读不到时不抛
- [369-bun-typeof-fetch-carries-preconnect](./369-bun-typeof-fetch-carries-preconnect.md) — 注入口写 `fetchFn: typeof fetch`，测试里的假实现 `bun test` 照跑，`tsc -p tsconfig.test.json` 报少一个 `preconnect`：`@types/bun` 的全局 `fetch` 是带静态属性的函数。注入口自己声明 `type FetchLike = (url: string, init?: RequestInit) => Promise<Response>`
...
- [382-ios-sim-port-is-only-where-the-script-looks](./382-ios-sim-port-is-only-where-the-script-looks.md) — `IOS_SIM_PORT` 只改 bridge 往哪敲 `eval`，vite 的端口在 `vite.config.ts` 的 `strictPort` 里；设了它 app 起来也永远 `waiting for the app to answer the bridge`。附：`cmd_up` 会 `pkill -f "tauri ios dev"`，并行会话互相收掉对方的 dev server
