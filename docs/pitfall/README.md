...
- [120-a-regex-tag-matcher-ends-the-tag-early](./120-a-regex-tag-matcher-ends-the-tag-early.md) — 正则清洗器把标签当成到第一个 `>` 为止，双引号属性值里的 `>` 让 `<marquee title="a>" onstart=...>` 整条穿过去；删属性替换成空串又把 `on` 和 `click=alert(1)` 粘成输入里没有的处理器。改成 DOMParser 解析 + 白名单走树 + 重新写出标签；测试用 jsdom 给 bun 补 DOMParser，判定用 `HTMLRewriter` 问浏览器看到什么
