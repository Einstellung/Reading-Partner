...
2026-09-09 在 iPad 模拟器（iOS 26.5 的 WKWebView）上量了同一组：SVG blob 交给 `createImageBitmap` 一样抛 `InvalidStateError: Cannot decode the data in the argument to createImageBitmap`；`<img>` 按 `viewBox` 报 400x200，`drawImage` 画出来中心像素是 `#3366cc`，`toDataURL("image/jpeg")` 出得来。两个引擎结论一致，`<img>` 那条是唯一能走的。
