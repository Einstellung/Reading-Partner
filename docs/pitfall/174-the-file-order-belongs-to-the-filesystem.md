...
3. 每文件一进程，带总数闸。三棵树给的答案一模一样，是唯一能转述的一个。`bash scripts/isolate.sh` 就是下面这段，加上并行和 react-dom 探针（坑 175）。
...
- `tests/reading/slides/deck-chapters.test.ts`（文件已删，随 slides/deck 功能一起下线）
