...
1. `src/platform/app/tauri-fetch.ts` 的 `cleanTauriFetch` 包住插件 fetch：abort 时把面向调用方的拒绝统一成标准 `AbortError`（插件本来抛的是 `Error("Request cancelled")` 或 resource-id 串）。papers/http 和 AI fetch 桥都走它。这只修调用方能拿到的那条 promise，管不到内部 `void` 掉的两个 invoke。
