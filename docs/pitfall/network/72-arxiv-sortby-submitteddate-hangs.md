...
区间是闭区间、分钟精度，所以"某年以来"也得给个上界（`src/info/sources/plugins/arxiv-client.ts` 用 2100）。这条路 0.75 秒的那档速度不变。
...
**限流按现成的来。** 主题检索走 `fetchWithRetry`（`src/platform/http/throttled-fetch.ts`），它有按 host 的最小间隔（arXiv 3000ms）和 429 退避，终态 429 抛 `RateLimitError`。检索工具把这个抛出报成"arXiv 这一趟没答上来"，其余三个库照常返回——四个库并发扇出，一个挂掉不能带走整次检索。
