...
分页横翻模式靠这个拿到滚动容器跟手改 scrollLeft、锁 touch-action：`src/reading/engine/EmbedPdfView.tsx` 的 `TouchInputRouter` 调 `useViewportElement()`，轮询到 `.current` 后交给 `src/reading/engine/gesture/attach-touch.ts` 的 `attachTouchRouter`。
