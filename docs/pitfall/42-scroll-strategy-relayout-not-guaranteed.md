...
解法：切换布局不要当成一次性命令。`setLayout` 把该模式的每一项（scroll axis / zoom / touch-action / 手势机状态 / fit-page 基准）无条件全量应用，不做 `mode === layout` 的提前返回，并在下一帧对 axis 和 zoom 再断言一次（`layout !== mode` 时放弃，让更新的那次切换赢）。设置项和"进出必须对称"的证明放在 `src/reader-embedpdf/layout-modes.ts` 的纯函数里，单测覆盖来回切换回到初始。
