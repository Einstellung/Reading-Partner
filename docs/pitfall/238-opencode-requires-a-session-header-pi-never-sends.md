...
映射表放 `src/ai/request-headers.ts`，一个 provider 一行，`opencode-go` 和 `opencode` 各一条。id 用会话 id，不是每次调用一个随机值——随机值也能让 400 消失，但把这个头存在的理由（路由和缓存）扔了。工具循环用 `TurnTelemetry.thread`，`runAgentTurn` 本来就为每一轮解析好了它，一次解析同时喂给埋点和这个头，于是一轮里的每个 round、同一会话的连续几轮，发出去的是同一个值；`streamChat` 由调用方传 `sessionId`，今天三个调用方都没有会话，各自现取一个新 id 并在调用点写明这是一次性调用。
...
手上没有 OpenCode 的 key，所以"服务端接受这个头"没有实测；测到的是请求确实带上了它。
