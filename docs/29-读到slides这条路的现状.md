...
图注这一半有三个成因。`captionLinesFromText`（:240）按基线 y 分行，不看 x 距离；双栏论文里左栏正文和右栏图注共享基线会被并成一行，图注不在行首，`FIG_RE`（:233）的 `^` 锚定失败，整条 caption 丢掉，索引里连条目都没有。`FIG_RE` 只捕获 `(\d+[a-z]?)`，"Figure 1.1" 和 "Figure 1.2" 拿到同一个 id `1`，`assembleIndex`（:536）按 id 去重，用章节编号的书每章只剩一张图；中文"图 1"和罗马数字不识别。第三处是 :251 的排序比较器不满足传递性（基线差 ≤3 按 x 比、否则按 y 比），基线密集时排序结果未定义，邻行的一个 run 插进来就能把一条图注切成两半、两半都匹配不上。
