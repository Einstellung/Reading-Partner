...
同步今天停在档 1：整棵树对拉，不推断删除。[59](../platform/59-同步：持有清单与裁决.md) 定了往上三档——档 2 按持有清单推断删除，档 3 线程按消息合并，档 4 冲突交给一个 agent 裁决。档 3 已落地（`src/platform/sync/merge/messages.ts`），混版期会抖，要单独发一版；档 4 已做（59 §6「落地」）。剩档 2。

## 为什么现在不做

档 2 翻开前要先补 `retired` 的生产端，翻早了会把路径读成删除。
