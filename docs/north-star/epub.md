现状见 [docs/64](../64-epub纸页.md)：EPUB 是桌上固定几何的纸页，和 PDF 同一套缩放与布局。手机不排纸页，读同一本书的重排列，见 [docs/70](../70-手机读EPUB.md)。
...
方案见 [39](../39-epub支持调研.md),渲染侧的实测结论见 [62](../62-epub渲染spike.md)。摄入(阶段 2)和接引擎(阶段 3)已落地,EPUB 能读;标注(阶段 4)桌上和手机都能划。手机形态按 [70](../70-手机读EPUB.md) 落地,在 iPhone 模拟器上跑通。还剩图的视觉描述(阶段 5)。
