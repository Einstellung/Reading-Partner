...
- 触发条件（2026-09-20 定）：concern 影响排序权重（[63](../info/63-情报局：研究室、专项组与态势.md)，因 LLM 太贵一直没做）要动手的时候。那时先把 `judge(state, questions) -> probabilities` 定成 capability，粗筛、回头重打、concern 排序、去重、饭的候选筛都走这一个口；接任何模型之前，先拿自己的 concern 标注在域外验一个小 encoder（Laya 或 GLiNER 2.5 那一档）。选 concern 排序当第一处，是因为「这条和 concern X 有没有关系」是蕴含形状的问题，encoder 最擅长、域外掉得最少。
- 粗筛不换：今天那道筛顺手产出 [65](../info/65-加工手册：常态模型、日更列表、滚雪球与稿.md) 要的一句话摘要和 outside 理由，判断模型写不出来，量也没到。
...
- 我们侧对得上的位置：粗筛（[63](../info/63-情报局：研究室、专项组与态势.md) 的命中下标加置信度，「模型只交下标不交字符串」就是这个形）、论文第 0/3/14 天回头重打（[69](../info/69-源模型：索引、叙事源、用户带来的与快照.md)）、concern 影响排序权重（63，因贵一直没落地）、分析员前的成对去重、饭的候选筛。
