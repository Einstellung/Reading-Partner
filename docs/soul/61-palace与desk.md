...
> 2026-09-08 定案。上游：[48](./48-记忆：观察与statement.md) 是观察与 statement 两个仓，[55](./55-legion.md) 是 run 与调度，[58](./58-蒸馏器与dream.md) 是蒸馏与回收，[59](../platform/59-同步：持有清单与裁决.md) 是同步与合并，[60](../info/60-info：白宫与Red Boxes.md) 是盒子与秘书，[21](../info/21-info收藏与reading打通.md) 是收藏的落法，[45](../companion/45-陪伴的形态.md) 是桌宠的形态。
>
> 「五个名词」和「顺序」两节已于 2026-09-11 由 [71](./71-soul.md) 取代：AI 改叫 soul，顺序按 71 那份。「登记」「定下的」「待定」三节照旧。
>
...
从 60 转来的三问已在 [63](../info/63-情报局：研究室、专项组与态势.md) 定案：打断算 `asked`；不要 `later`；过期按同研究室的新盒取代。
