...
> 2026-09-07 定案。在 [48](./48-记忆：观察与statement.md) 之上加一层，不改它的任何结论。legion 侧在 [55](./55-legion.md)。删除规则在 [50](../platform/50-删除.md)。
...
运行器是一种 legion run kind：脱手，靠 claim 选举保证同一时刻只有一台设备在跑（选举从 info 采集端那套泛化，见 55），失败落进 legion 的 ledger（失败事件已带 `errorName` / `errorMessage`）。今天那个 30 分钟扫描器变成 legion/schedule 上的一条。
