...
> 2026-09-07 定案。同步引擎与删除模型在 [13](./13-账户同步.md) 和 [50](./50-删除.md)，记忆的两个仓在 [48](./48-记忆：观察与statement.md)。回收（gc）归 memory，不在本文。info 管线重做的那份还没写，写出来时 [17](./17-信息源系统.md) 作废。
...
`legion/presence`，每设备一个心跳文件，广告自己的能力。租约算法从 `info/briefing/{presence,handoff}.ts` 泛化。
