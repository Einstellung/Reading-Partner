...
> 2026-09-12 定案。上游：[64](./64-epub纸页.md) 是纸页、分页表与字体，[21](../info/21-info收藏与reading打通.md) 是收藏的入口与确认卡，[09](./09-学习机模式.md) 是链接摄入（`ingest_url`），[61](../soul/61-palace与desk.md) 是 topic 与 desk，[55](../soul/55-legion.md) 是 run 与调度。
>
> 取代关系：[60](../info/60-info：白宫与Red Boxes.md)「出口：稿」里的渲染与引用那几条作废（react-markdown 渲染壳、没有页、竖滚、按节引用、划线存节号+偏移、图引源站 URL），稿改走本文。21 和未做清单里挂着的「收藏的文章算不算书」在本文答完。
...
两个：书或 topic 根聊天里贴 URL；iOS 分享面板进 app。书内贴入落成这本书的辅助资料，不问 topic，见下一节。topic 根聊天落到 21 那张确认卡：归到哪个 topic，加一句它对这个 topic 加了什么，点头才落盘进 topic，还没做。分享面板进来的链接进 Lumen 的对话，见 [78](../platform/78-分享.md)。
...
翻译：说「这篇翻一下」时不给标题就翻屏幕上那份（supplement 或书），给标题先在这本书的 supplement 里找。译本替换原文档那条路对 supplement 是换列表里的条目（旧 hash 拿掉、新 hash 加上，标题是新文件名去扩展名），划线和线程照文档 id 搬，旧文档走删书。读者正看着它就在会话内换成新的一份。

删除：读者说「把这篇资料删掉」，AI 调 `remove_supplement(title)`——从列表拿掉、删那份文档、prep 里对应的 paper 标 skipped，正看着就先换回书。Outline 上没有删除按钮，纠错走对话。
...
翻译换的是 `documentId`，笔记不重做：省一次 digest 调用，代价是笔记里的页码按原文数的，双语版里可能差一两页。
