...
- 分层：platform（`platform/app`/`platform/sync`，`platform/app` 不 import 任何别的目录）→ capability（`ai`/`ai/voice`/`fulltext`，headless，只被领域调用，绝不反向 import 领域）→ 领域（`reading` 及其子目录 `engine`/`prep`/`notes`/`figures`/`slides`、`info`、`memory`，互相可用但目录级依赖图必须无环）→ `ui/components` → `App.tsx` → 入口（`main.tsx`/`smoke`）。领域的编排代码放自己的领域目录，不要塞进 capability。规则由 `tests/layering.test.ts` 强制，新增目录（顶层的，或分组目录下的）必须在那里的 LAYER 表里登记。
