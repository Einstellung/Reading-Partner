...
import { createEventLogger, formatEventLine } from "../src/platform/app/events";
...
test("logger appends per topic with the injected clock; payload defaults empty", async () => {
  ...
  expect(JSON.parse(appended[1].line)).toEqual({ ts: 42, type: "prep-tab-open" });
});
