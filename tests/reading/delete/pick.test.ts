...
test("the local paths cover the synced ones, the caches and the blob", () => {
  ...
  expect(files).toEqual([
    ...
    `figures-${BOOK}.json`,
    `library/${BOOK}.pdf`,
    // The cover and the marker that says why there is no cover: a deleted book
    // must not be a picture on the next shelf, and covers are filed under the
    // book id, so re-importing the same PDF would otherwise show the old one.
    `covers/${BOOK}.jpg`,
