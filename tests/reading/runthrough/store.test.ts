// The run-through log on disk (src/reading/runthrough/store.ts): the round
...
import type { RunthroughRun } from "../../../src/reading/runthrough/types";
...
function aRun(id: string, over: Partial<RunthroughRun> = {}): RunthroughRun {
  return {
    id,
    ordinal: 1,
    talkId: TALK,
...
test("a talk that has never been given reads as an empty log, not an error", async () => {
  const log = await loadRunthroughs(TALK);
  expect(log.runs).toEqual([]);
  expect(log.talkId).toBe(TALK);
  expect(app.files.has(runthroughFile(TALK))).toBe(false);
});
...
test("one talk's runs are not another's", async () => {
  ...
  expect((await loadRunthroughs(TALK)).runs.map((r) => r.id)).toEqual(["r1"]);
  expect((await loadRunthroughs("talk-2")).runs.map((r) => r.id)).toEqual(["other"]);
});
...
test("a file that will not parse is moved aside before the empty log is handed back", async () => {
  app.files.set(runthroughFile(TALK), "{not json");
  const log = await loadRunthroughs(TALK);
  expect(log.runs).toEqual([]);
  expect(renamed).toEqual([`${runthroughFile(TALK)} -> ${runthroughFile(TALK)}.bad`]);
  expect(app.files.get(`${runthroughFile(TALK)}.bad`)).toBe("{not json");
  expect(app.files.has(runthroughFile(TALK))).toBe(false);
});
...
test("a version this build does not know is set aside, not read as empty in place", async () => {
  app.files.set(runthroughFile(TALK), JSON.stringify({ version: 99, talkId: TALK, runs: [] }));
  expect((await loadRunthroughs(TALK)).runs).toEqual([]);
...
test("a file that would not open is left alone", async () => {
  app.files.set(runthroughFile(TALK), JSON.stringify({ version: 1, talkId: TALK, runs: [] }));
  app.unreadable.add(runthroughFile(TALK));
  expect((await loadRunthroughs(TALK)).runs).toEqual([]);
  expect(renamed).toEqual([]);
  expect(app.files.has(runthroughFile(TALK))).toBe(true);
});
...
test("deleting takes the log and its rescue copy with it", async () => {
  await appendRun(aRun("r1"));
  app.files.set(`${runthroughFile(TALK)}.bad`, "{not json");
  await deleteRunthroughs(TALK);
  expect(app.files.has(runthroughFile(TALK))).toBe(false);
  expect(app.files.has(`${runthroughFile(TALK)}.bad`)).toBe(false);
});
