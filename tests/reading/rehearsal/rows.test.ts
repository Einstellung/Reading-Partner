...
// the rehearsals on disk and the retells whose talk nobody has given yet, and
...
test("a retell whose deck has never been given is a row with no rehearsal behind it", () => {
  const rows = rehearsalRows(
    [],
    [{ retellId: "900", name: "Eye and Brain", outlineId: "o-900" }],
    NO_RUNS,
  );
  expect(rows).toHaveLength(1);
  expect(rows[0].id).toBeNull();
  expect(rows[0].retellId).toBe("900");
  expect(rows[0].outlineId).toBe("o-900");
...
test("newest first, whether or not the rehearsal exists yet", () => {
  const rows = rehearsalRows(
    ...
    [{ retellId: "2000", name: "Middle", outlineId: "o-2000" }],
    NO_RUNS,
  );
...
test("keys are unique across the two kinds of row", () => {
  const rows = rehearsalRows(
    ...
    [{ retellId: "900", name: "A retell that happens to share the number", outlineId: "o-900" }],
    NO_RUNS,
  );
