...
#
# One uncommitted working-tree edit, reverted by long-run-finish.sh:
# measurement-patch.py (backstop 1500 s plus 30-second telemetry). It must
...
sudo -A launchctl asuser "$GUI_UID" sudo -u "$GUI_USER" \
  /bin/bash -lc "cd '$REPO' && export PATH='$PATH' APPLE_DEVELOPMENT_TEAM=$TEAM \
    ...
    APPLE_API_KEY_PATH=$APPLE_API_KEY_PATH VITE_SMOKE=dictation-long && \
