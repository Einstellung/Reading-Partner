#!/bin/bash
# Build the twenty-minute hold as a measurement build and put it on the phone.
#
# Two uncommitted working-tree edits, both reverted by long-run-finish.sh:
# the .dev bundle id (local signing) and measurement-patch.py (backstop 1500 s
# plus 30-second telemetry). Neither may reach a commit.
set -euo pipefail
...
step "measurement patch"
python3 /tmp/measurement-patch.py apply "$REPO"
grep -n "backstopSeconds" plugins/voice/ios/Sources/DictationRun.swift | head -2
...
step "build (VITE_SMOKE=dictation-long)"
sudo -A launchctl asuser "$GUI_UID" sudo -u "$GUI_USER" \
  /bin/bash -lc "cd '$REPO' && export PATH='$PATH' APPLE_DEVELOPMENT_TEAM=$TEAM VITE_SMOKE=dictation-long && bun run tauri ios build --debug --target aarch64" 2>&1 | tail -4
...
step "console"
bash /tmp/syslog.sh /tmp/rp-long.log

step "battery before"
ideviceinfo -u "$DEVICE" -q com.apple.mobile.battery -k BatteryCurrentCapacity
ideviceinfo -u "$DEVICE" -q com.apple.mobile.battery -k ExternalConnected

step "launch"
xcrun devicectl device process launch --device "$DEVICE" "$DEV_ID" 2>&1 | tail -2
echo "launched at $(date +%H:%M:%S) — waiting for one tap"
