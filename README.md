...
Highlights, underlines, the AI pen, ink, the list of what you marked, notes, the observations a conversation leaves, and Drive sync are the same on both formats. A text mark on an EPUB is stored as a CFI with the quoted sentence and its neighbours beside it, so it is found again by its words if the markup shifts under it; ink is stored in page coordinates like on a PDF. Figures are the book's own images with their `<figcaption>` as the caption, so the AI looks at the picture the book shipped.
...
- `src/reading/` — the book half: `engine/` is the EmbedPDF adapter (assembles the headless core + plugins, renders from in-memory bytes, converts annotations at the boundary), `epub/` is the other engine (unzip, sanitize, lay out into fixed paper pages, CFI marks, behind the same reader contract), `prep/` prepares a document in one of two kinds (`papers/` for a survey's citations, `chapters/` for a book's chapter spines), `papers/` is the citation graph and the four literature clients, `chapters/` is the book's own chapter table, `lecture/` decides what a teaching turn is handed, `figures/` crops figures off the page, `sources/` fetches and extracts a pasted link, and `retell/`, `talk/` and `rehearsal/` are the second stage — the questioning, the outline it settles, and giving the talk out loud.
...
- `public/pdfium/pdfium.wasm` — the PDFium engine binary, self-hosted (gitignored; staged by `bun run wasm` from the npm package, no CDN at build or runtime). `vendor/foliate-js/` — the CFI and text-walking pieces of foliate-js, copied from a pinned upstream commit (MIT). `public/fonts/` — the Noto Serif faces an EPUB is laid out in (OFL). `src-tauri/` — Tauri 2 app, including the tray and the hidden-webview fetch. `plugins/voice/` — the Tauri plugin behind recording and the phone's on-device dictation.
...
Copyright (c) 2026 Xinyuan ([Einstellung](https://github.com/Einstellung)). Source-available under the [PolyForm Strict License 1.0.0](./LICENSE): you may use it for personal and noncommercial purposes, but not redistribute it, modify it, or use it commercially; the only place to get it is this repository's Releases and TestFlight, and anything else needs a separate license — contact einstellungsu@gmail.com. The PDF engine is [EmbedPDF](https://github.com/embedpdf/embed-pdf-viewer) (MIT), which renders through [PDFium](https://pdfium.googlesource.com/pdfium/) compiled to WebAssembly (Apache-2.0). EPUB positions use the CFI code of [foliate-js](https://github.com/johnfactotum/foliate-js) (MIT), vendored in `vendor/foliate-js/`; EPUB pages are set in Noto Serif and Noto Serif CJK (SIL Open Font License).
