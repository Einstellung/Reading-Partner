...
- `src/ai/` — provider streaming and the agent tool loop. `src/reading/prep/` — the lesson-prep pipeline, with `prep/sources/` for link ingestion. `src/reading/papers/` — academic literature search and the citation graph. `src/memory/` — the per-topic memory store. `src/reading/figures/` — figure extraction and rendering. `src-tauri/` — Tauri 2 app.
